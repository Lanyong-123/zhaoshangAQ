(function(){
  ...
  var keys=["assessmentNo","orgType","orgName","cycle","assessmentForm","publisher","publisherOrg","l1No","l1","l2No","l2","l3No","l3","l4No","l4","l5No","l5","standard","required","scoring","problemType","planDate","dueStatus","date","material","note","status","auditStatus"];
  var headers=["评估编号","所属组织/业态","组织/项目名称","评估周期","评估形式","任务发布人","任务发布人所属组织","一级要素序号","一级要素名称","二级要素序号","二级要素名称","三级要素序号","三级要素名称","四级要素序号","四级要素名称","五级要素序号","五级要素名称","达标标准","必须项","评分方式","隐患类型","计划整改完成时间","到期状态","整改时间","整改资料","整改说明","整改完成情况","审核状态"];
  ...
  function reset(){["keyword","orgType","orgLevel","problemType","assessmentForm","publisherOrg","cycleStart","cycleEnd","required","scoring","status","dueStatus","auditStatus","dataStatus","planEndDate","implementStartDate","implementEndDate","startDate","endDate"].forEach(function(id){document.getElementById(id).value=""});setDefaultDueDate();selectedOrg="";document.querySelectorAll(".tree-node.active").forEach(function(node){node.classList.remove("active")});document.querySelector(".tree-node[data-org='']").classList.add("active");render(rows);toast("查询条件已重置")}
