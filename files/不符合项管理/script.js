(function(){
  ...
  function reset(){["keyword","orgType","orgLevel","problemType","assessmentForm","publisherOrg","cycleStart","cycleEnd","required","scoring","status","dueStatus","dataStatus","elementName","planEndDate","implementStartDate","implementEndDate","startDate","endDate"].forEach(function(id){document.getElementById(id).value=""});setDefaultDueDate();selectedOrg="";document.querySelectorAll(".tree-node.active").forEach(function(node){node.classList.remove("active")});document.querySelector(".tree-node[data-org='']").classList.add("active");render(rows);toast("查询条件已重置")}
