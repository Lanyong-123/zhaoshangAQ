(function(){
  ...
  var dataStatuses=["可用","可用","可用","不可用","可用","可用","不可用","可用"];
  rows=rows.map(function(row,index){row.dataStatus=dataStatuses[index]||"可用";return row});
  ...
  function reset(){["keyword","orgType","orgLevel","problemType","cycleStart","cycleEnd","required","scoring","status","dueStatus","auditStatus","dataStatus","levelOne","planStartDate","planEndDate","startDate","endDate"].forEach(function(id){document.getElementById(id).value=""});setDefaultDueDate();selectedOrg="";document.querySelectorAll(".tree-node.active").forEach(function(node){node.classList.remove("active")});document.querySelector(".tree-node[data-org='']").classList.add("active");render(rows);toast("查询条件已重置")}
