(function () {
  ...
  function render(data){
    ...
    document.getElementById("summaryBody").innerHTML=types.map(function(type){
      var items=data.filter(function(r){return r.type===type}),total=items.reduce(function(n,r){return n+r.total},0),completed=items.reduce(function(n,r){return n+r.completed},0),unfinished=total-completed;
      return '<tr><td>'+type+'</td><td>'+statLink(total,type,"隐患总数")+'</td><td>'+statLink(completed,type,"已完成整改数")+'</td><td>'+statLink(unfinished,type,"未完成整改数")+'</td><td>'+esc(rate(completed,total))+'</td></tr>';
    }).join("");
  ...
  function query(){var org=document.getElementById("orgName").value.trim().toLowerCase(),type=document.getElementById("issueType").value,start=document.getElementById("statStart").value,end=document.getElementById("statEnd").value,scopes=checkedValues("orgTreeSelect"),levels=checkedValues("orgLevelSelect");render(rows.filter(function(r){return(!org||r.org.toLowerCase().indexOf(org)>-1)&&(!type||r.type===type)&&(!start||r.date>=start)&&(!end||r.date<=end)&&matchesScope(r,scopes)&&(!levels.length||levels.indexOf(String(r.level))>-1)}));toast("查询统计完成，共 "+current.length+" 条明细")}
  function reset(){["orgName","issueType","statStart","statEnd"].forEach(function(id){document.getElementById(id).value=""});document.querySelectorAll(".multi-panel input").forEach(function(input){input.checked=false});updateMultiLabel("orgTreeSelect","请选择组织范围");updateMultiLabel("orgLevelSelect","请选择组织层级");render(rows);toast("查询条件已重置")}
  ...
  document.getElementById("queryBtn").onclick=query;document.getElementById("resetBtn").onclick=reset;document.getElementById("exportBtn").onclick=exportCsv;document.getElementById("orgName").onkeydown=function(e){if(e.key==="Enter")query()};render(rows);
})();
