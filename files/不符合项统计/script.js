(function () {
  ...
  function query(){var org=document.getElementById("orgName").value.trim().toLowerCase(),type=document.getElementById("issueType").value,start=document.getElementById("statStart").value,end=document.getElementById("statEnd").value,dueDate=document.getElementById("dueDate").value,scopes=checkedValues("orgTreeSelect"),level=document.getElementById("orgLevel").value;render(rows.filter(function(r){return(!org||r.org.toLowerCase().indexOf(org)>-1)&&(!type||r.type===type)&&(!start||r.date>=start)&&(!end||r.date<=end)&&(!dueDate||r.date<=dueDate)&&matchesScope(r,scopes)&&(!level||String(r.level)===level)}));toast("查询统计完成，共 "+current.length+" 条明细")}
  ...
  function reset(){["orgName","issueType","statStart","statEnd","orgLevel","dueDate"].forEach(function(id){document.getElementById(id).value=""});setDefaultDueDate();document.querySelectorAll(".multi-panel input").forEach(function(input){input.checked=false});updateMultiLabel("orgTreeSelect","请选择组织范围");render(rows);toast("查询条件已重置")}
