(function () {
  ...
  function emptyStats(){return {total:0,due:0,completed:0,expiredCompleted:0,overdue:0,notDue:0,notDueCompleted:0}}
  ...
  function render(data){
    ...
    document.getElementById("summaryBody").innerHTML=types.map(function(type){
      var items=data.filter(function(r){return r.type===type}),
        ...
        onTimeCompleted=items.reduce(function(n,r){return n+Math.max(r.completed-(r.expiredCompleted||0),0)},0),
      ...
      return '<tr><td>'+type+'</td><td>'+statLink(total,type,"隐患总数")+'</td><td>'+statLink(onTimeCompleted,type,"按期整改数")+'</td><td>'+statLink(expiredCompleted,type,"超期整改数")+'</td><td>'+statLink(notDueUnfinished,type,"未到期未整改数")+'</td><td>'+statLink(overdueUnfinished,type,"超期未整改数")+'</td></tr>';
    }).join("");
    var orgRows=orgSummary(data);
    document.getElementById("detailBody").innerHTML=orgRows.length?orgRows.map(function(r){var total=["重大隐患","集团负面清单","公司管理红线"].reduce(function(sum,type){return sum+((r.types[type]&&r.types[type].total)||0)},0);return '<tr><td>'+esc(r.org)+'</td><td>'+statLink(total,"全部","累计隐患总数",r.org)+'</td>'+typeCells(r,"重大隐患")+typeCells(r,"集团负面清单")+typeCells(r,"公司管理红线")+'</tr>'}).join(""):'<tr><td class="empty" colspan="17">未查询到符合条件的统计数据</td></tr>';
  ...
  function query(){var org=document.getElementById("orgName").value.trim().toLowerCase(),type=document.getElementById("issueType").value,assessmentForm=document.getElementById("assessmentForm").value,usableStatus=document.getElementById("usableStatus").value,start=document.getElementById("statStart").value,end=document.getElementById("statEnd").value,dueDate=document.getElementById("dueDate").value,implementStart=document.getElementById("implementStart").value,implementEnd=document.getElementById("implementEnd").value,scopes=checkedValues("orgTreeSelect"),level=document.getElementById("orgLevel").value;render(rows.filter(function(r,index){var implementDate=r.implementDate||r.date,form=r.assessmentForm||"监督评审",taskScope=rowTaskScope(r,index),status=r.usableStatus||"数据可用";return(!activeTaskScope||activeTaskScope==="all"||taskScope===activeTaskScope)&&(!org||r.org.toLowerCase().indexOf(org)>-1)&&(!type||r.type===type)&&(!assessmentForm||form===assessmentForm)&&(!usableStatus||status===usableStatus)&&(!start||r.date>=start)&&(!end||r.date<=end)&&(!dueDate||r.date<=dueDate)&&(!implementStart||implementDate>=implementStart)&&(!implementEnd||implementDate<=implementEnd)&&matchesScope(r,scopes)&&(!level||String(r.level)===level)}))}
  ...
  function exportCsv(){var headers=["组织名称","累计隐患总数（累计求和，动态更新）","重大事故隐患数","重大事故隐患按期整改数","重大事故隐患超期整改数","重大事故隐患未到期未整改数","重大事故隐患超期未整改数","集团负面清单数","集团负面清单按期整改数","集团负面清单超期整改数","集团负面清单未到期未整改数","集团负面清单超期未整改数","公司管理红线数","公司管理红线按期整改数","公司管理红线超期整改数","公司管理红线未到期未整改数","公司管理红线超期未整改数"],orgRows=orgSummary(current),rowValues=function(row){var result=[row.org],total=["重大隐患","集团负面清单","公司管理红线"].reduce(function(sum,type){return sum+((row.types[type]&&row.types[type].total)||0)},0);result.push(total);["重大隐患","集团负面清单","公司管理红线"].forEach(function(type){var item=row.types[type]||emptyStats(),onTimeCompleted=Math.max(item.completed-item.expiredCompleted,0),overdueCompleted=item.expiredCompleted,notDueUnfinished=Math.max(item.notDue-item.notDueCompleted,0),overdueUnfinished=item.overdue||0;result.push(item.total,onTimeCompleted,overdueCompleted,notDueUnfinished,overdueUnfinished)});return result},csv="\ufeff"+headers.join(",")+"\n"+orgRows.map(function(r){return rowValues(r).map(function(v){return '"'+String(v).replace(/"/g,'""')+'"'}).join(",")}).join("\n"),blob=new Blob([csv],{type:"text/csv;charset=utf-8"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="不符合项统计_"+new Date().toISOString().slice(0,10)+".csv";a.click();URL.revokeObjectURL(a.href)}
  function exportListCsv(){if(!current.length){return}var headers=["组织名称","组织层级","评估形式","隐患类型","隐患总数","按期整改数","超期整改数","未到期未整改数","超期未整改数","任务发布时间","任务实施时间"],csv="\ufeff"+headers.join(",")+"\n"+current.map(function(r){var notDue=r.total-r.due,notDueCompleted=Math.max(r.completed-r.due,0),notDueUnfinished=Math.max(notDue-notDueCompleted,0),onTimeCompleted=Math.max(r.completed-(r.expiredCompleted||0),0),implementDate=r.implementDate||r.date,form=r.assessmentForm||"监督评审",values=[r.org,r.level+"级",form,r.type,r.total,onTimeCompleted,r.expiredCompleted||0,notDueUnfinished,r.overdue,r.date,implementDate];return values.map(function(v){return '"'+String(v==null?"":v).replace(/"/g,'""')+'"'}).join(",")}).join("\n"),blob=new Blob([csv],{type:"text/csv;charset=utf-8"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="不符合项清单_"+new Date().toISOString().slice(0,10)+".csv";a.click();URL.revokeObjectURL(a.href)}
